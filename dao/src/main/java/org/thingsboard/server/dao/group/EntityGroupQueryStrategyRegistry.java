...
@Component
public class EntityGroupQueryStrategyRegistry {

    private final Map<EntityType, EntityGroupQueryStrategy<?>> registry;

    @Autowired
    public EntityGroupQueryStrategyRegistry(List<EntityGroupQueryStrategy<?>> strategies) {
        this.registry = strategies.stream()
                .collect(Collectors.toMap(EntityGroupQueryStrategy::supportedType, Function.identity()));
    }
