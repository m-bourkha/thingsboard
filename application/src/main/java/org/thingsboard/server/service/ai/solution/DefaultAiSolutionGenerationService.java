...
public class DefaultAiSolutionGenerationService implements AiSolutionGenerationService {
    ...
    public ListenableFuture<AiSolutionSpec> generate(TenantId tenantId, AiModelId modelId, String prompt) {
        return sendAndParse(tenantId, modelId, AiSolutionPrompts.generateUserPrompt(prompt));
    }
    ...
    public ListenableFuture<AiSolutionSpec> refine(TenantId tenantId, AiModelId modelId, AiSolutionSpec currentSpec, String message) {
        ...
        return sendAndParse(tenantId, modelId, AiSolutionPrompts.refineUserPrompt(currentSpecJson, message));
    }

    private ListenableFuture<AiSolutionSpec> sendAndParse(TenantId tenantId, AiModelId modelId, String userPrompt) {
        return aiModelService.findAiModelByTenantIdAndIdAsync(tenantId, modelId)
                .transformAsync(modelOpt -> {
                    ...
                    return sendChatRequest(model, userPrompt);
                }, directExecutor())
                .transform(chatResponse -> parseSpec(chatResponse.aiMessage().text()), directExecutor());
    }

    private <C extends AiChatModelConfig<C>> FluentFuture<ChatResponse> sendChatRequest(AiModel model, String userPrompt) {
        ...
        ChatRequest.Builder requestBuilder = ChatRequest.builder()
                .messages(List.<dev.langchain4j.data.message.ChatMessage>of(
                        SystemMessage.from(AiSolutionPrompts.SYSTEM_PROMPT),
